    ...
    while(m_run)
    {
        if (m_session == NULL)
        {
            if (m_host != "")
            {
                ...
                ssh_options_set(m_session, SSH_OPTIONS_HOST, m_host.toLocal8Bit().data());
                ssh_options_set(m_session, SSH_OPTIONS_USER, m_user.toLocal8Bit().data());
        ...
        else if (!m_loggedIn)
        {
            ...
            if (m_user != "" && m_password !="")
            {
                ...
                int worked = ssh_userauth_password(m_session, m_user.toLocal8Bit().data(), m_password.toLocal8Bit().data());
        ...
        else if (!m_currentOperation.executed)
        {
            ...
            if (m_currentOperation.type == Command || m_currentOperation.type == WorkingDirectoryTest)
            {
                ...
                while (requestResponse == SSH_AGAIN)
                    requestResponse = ssh_channel_request_exec(channel, m_currentOperation.adminCommand.toLocal8Bit().data());
            ...
            {
                ssh_scp scpSession = ssh_scp_new(m_session,SSH_SCP_READ, m_currentOperation.remotePath.toLocal8Bit().data());
                if (scpSession == NULL)
            ...
            else if (m_currentOperation.type == Push)
            {
                ...
                ssh_scp scpSession = ssh_scp_new(m_session,SSH_SCP_WRITE, m_currentOperation.remotePath.toLocal8Bit().data());
                ...
                if (ssh_scp_push_file(scpSession, m_currentOperation.remotePath.toLocal8Bit().data(), buffer.size(), S_IRUSR | S_IWUSR) != SSH_OK)
                {
                    ssh_scp_close(scpSession);
                    ssh_scp_free(scpSession);
...
bool QSshSocket::isExecuted(void)
{
    return is_executed;
}
